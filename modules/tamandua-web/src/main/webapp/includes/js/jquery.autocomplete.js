...
function urlSearchEncode(clearString){
	var output  = '';
	clearString = (clearString != undefined) ? clearString.toString() : "";
	output = clearString.replace(/%2F/g, "/");
	return output;

	//output = clearString.replace(/[#\?;\*%\"]/g, "");
	//return output;
	//return clearString;
}

function urlEncode( s ){
	//  return urlSearchEncode(unescape( encodeURIComponent( s ) ));
    return urlSearchEncode(encodeURIComponent( s ));
}
...
$.Autocompleter = function(input, options) {
	...
	$input.bind(($.browser.opera ? "keypress" : "keydown") + ".autocomplete", function(event) {
		...
		// trsack whether the field has focus, we shouldn't process any
	...
	function selectCurrent() {
		...
		$input.val(selected.result.text());
		$hidden.val(selected.value.text());
	...
	function request(term, success, failure) {
		...
		// if an AJAX url has been supplied, try loading the data now
			...
			$.ajax({
				...
				url: options.url + "?" + input.id + "=" + input.value,
