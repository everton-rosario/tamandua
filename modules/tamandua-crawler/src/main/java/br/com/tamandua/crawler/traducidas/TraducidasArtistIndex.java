...
/**
 * Class TraducidasArtistIndex
 *
 * @author Everton Rosario (erosario@gmail.com)
 */
public class TraducidasArtistIndex implements IProviderArtistIndex {
    private static final String PROVIDER = "traducidas";
    ...
    public ArtistList crawlLetter(String letter, CrawlerHTTP http) throws IOException {
        ...
        String responseBody = http.getResponseBody("/bandas/"+letter.toUpperCase());
        AZTraducidasParser parser = new AZTraducidasParser(responseBody);
    ...
    public void crawlTracksIndexFromArtists(ArtistList artistList) throws JAXBException, IOException, InterruptedException {
    	...
    	long end = System.currentTimeMillis();
    	System.out.println("traducidas.crawlTracksIndexFromArtists took ["+(end - start)+"] miliseconds.");
	...
	private TrackList crawlTracksIndexFromArtist(Artist artist, CrawlerHTTP http) throws IOException, InterruptedException {
		...
		if (!alreadyCrawled) {
    		...
    		while (!ok) {
    			try {
    				responseBody = http.getResponseBody("/letras" + artist.getUrl().substring(0, artist.getUrl().length() - 1));
    				ok = true;
    			} catch (Exception ex) {
    			    while(!ok) {
    			        try {
	...
	public TrackReport crawlTrackContents(ArtistList fullArtistList, int numberThreads, String type) throws JAXBException, InterruptedException {
		...
		long end = System.currentTimeMillis();
		System.out.println("TraducidasArtistIndex.crawlTrackContents took [" + (end - start) + "] milliseconds.");
		return report;
	}
